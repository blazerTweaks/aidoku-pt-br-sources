...
fn parse_manga_list(html: &Document) -> Vec<Manga> {
	...
	if let Some(items) = html.select(".page-item-detail") {
		for item in items {
			...
			let manga_status = if status.to_lowercase().contains("completo")
				|| status.to_lowercase().contains("finalizado")
					...
					if label.contains("status") {
						if let Some(status) = item.select_first(".summary-content").and_then(|e| e.text()) {
							manga.status = match status.to_lowercase().as_str() {
								...
								s if s.contains("completed") || s.contains("finalizado") || s.contains("completo") => MangaStatus::Completed,
								s if s.contains("hiatus") || s.contains("hiato") => MangaStatus::Hiatus,
								s if s.contains("cancelled") || s.contains("cancelado") => MangaStatus::Cancelled,
...
impl ListingProvider for LerMangas {
...
impl DeepLinkHandler for LerMangas {
...
register_source!(LerMangas, ListingProvider, DeepLinkHandler);
